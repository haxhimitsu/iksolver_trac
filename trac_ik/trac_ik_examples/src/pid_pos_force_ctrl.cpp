...
class pos_force_controller{
  ...
  private:
    ...
    vector<double> default_force_pid_gain{0.0002*1,0.0,0.000024*1.0};//{P,I,D} for ex
    // vector<double> default_force_pid_gain{0.00018*5.5,0.001,0.000024*1};//{P,I,D} for ex2
    ...
    vector<double> default_torque_pid_gain{0.01*1,0.0,0.0024*2};//{P,I,D} for ex2
...
pos_force_controller::pos_force_controller():nh(), tfBuffer_(), tfListener_(tfBuffer_)
{
  ...
  geometry_msgs::TransformStamped force_transform;
    ...
    current_force=check_outlines(temp,30.0,-30.0);
